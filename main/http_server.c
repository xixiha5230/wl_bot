...
static esp_err_t set_handler(httpd_req_t *request)
{
    ...
    /* Yaw heading fusion tuning: ?yawscale=<deg/s per rad/s>&yawcorr=<1/s>. */
    {
        float scale, corr;
        robot_control_get_yaw_wheel(&scale, &corr);
        bool any = false;
        if (httpd_query_key_value(query, "yawscale", value, sizeof(value)) == ESP_OK) {
            scale = strtof(value, NULL);
            any = true;
        }
        if (httpd_query_key_value(query, "yawcorr", value, sizeof(value)) == ESP_OK) {
            corr = strtof(value, NULL);
            any = true;
        }
        if (any) {
            ...
            snprintf(applied + strlen(applied), sizeof(applied) - strlen(applied),
                     "yawscale=%.2f yawcorr=%.2f ", scale, corr);
        }
    }
    ...
    /* One-shot leg bump: ?bump=1|2 fires it; bumpamp/bumpms/bumpspeed tune it. */
    {
        ...
        if (httpd_query_key_value(query, "bumpacc", value, sizeof(value)) == ESP_OK) {
            acc = atoi(value);
            any = true;
        }
        if (any) {
            ...
            robot_control_get_bump_params(&amp, &ticks, &speed, &acc);
            snprintf(applied + strlen(applied), sizeof(applied) - strlen(applied),
                     "bump amp=%d ms=%d speed=%d acc=%d ", amp, ticks, speed, acc);
        }
    }
